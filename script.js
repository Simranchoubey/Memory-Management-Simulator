...
var allocationType = "best"; 
// options: "first", "best", "worst"
function Process(size, time) {
	this.size = size;
	this.timeLeft = time;
	this.allocatedBlock = null;
...
function Heap() {
  ...
  this.requestAllocation = function(process) {
    ...
    spaceLeftover = selectedBlock.size - (process.size + memControlBlockSize);

    if(spaceLeftover > 0){

        newBlock = new MemControlBlock(spaceLeftover);

        nextBlock = selectedBlock.next;
    ...
    selectedBlock.setProcess(process);
    process.allocatedBlock = selectedBlock;
...
// Handle front-end process submission
document.getElementById("processForm").onsubmit = function () {
	allocationType = document.getElementById("algorithm").value;
	elements = this.elements; // Form elements
